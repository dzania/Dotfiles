    ...
    fish --private --command "
        ...
        for fetch in $hydro_fetch false
            ...
            if test \"\$ahead\" -gt 0 2>/dev/null
                set files_changed (command git diff --name-only @{upstream}...@ 2>/dev/null | command wc -l | string trim)
                test -n \"\$files_changed\" && test \"\$files_changed\" -gt 0 && set files_changed \" $hydro_symbol_git_files_changed\$files_changed\"
                or set files_changed \"\"
            end

            switch \"\$behind \$ahead\"
                case \" \" \"0 0\"
                case \"0 *\"
                    set upstream \" $hydro_symbol_git_ahead\$ahead\$files_changed\"
                case \"* 0\"
                    set upstream \" $hydro_symbol_git_behind\$behind\"
                case \*
                    set upstream \" $hydro_symbol_git_ahead\$ahead\$files_changed $hydro_symbol_git_behind\$behind\"
            end
...
set --query hydro_symbol_git_files_changed || set --global hydro_symbol_git_files_changed ~
